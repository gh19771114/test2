...
function PartnersNetwork({ partners, partnerIcons }: { partners: Partner[], partnerIcons: PartnerIcon[] }) {
  ...
  // 根据设备类型设置top值
  const redBoxTop = deviceType === 'desktop' ? '10px' : '20px'
  const blueBoxTop = '20px'
  const redBoxHeight = deviceType === 'desktop' ? 'calc(100% - 10px)' : 'calc(100% - 20px)'
  const blueBoxHeight = 'calc(100% - 20px)'
  ...
  // 中心点（容器正中心）
  const centerX = containerWidth / 2.0
  const centerY = containerHeight / 2.0
  // 卡片尺寸
  const cardWidth = 240
  ...
  const cardDiagonal = Math.sqrt(maxCardWidth * maxCardWidth + maxCardHeight * maxCardHeight) / 2.0
  const padding = 40 // 增加边距，避免卡片互相遮挡
  const maxRadius = Math.min(
    (containerWidth / 2.0) - cardDiagonal - padding,
    (containerHeight / 2.0) - cardDiagonal - padding
  )
  // 计算相邻卡片所需的最小半径，确保不遮挡
  // 9个卡片，每个间隔40度，需要确保相邻卡片中心距离 > 卡片对角线 + margin
  const numCards = partners.length
  const angleStep = 360.0 / numCards // 40度
  const minCardDistance = cardDiagonal * 2 + 20 // 卡片对角线 + 边距（减少到20以适配容器）
  const requiredRadius = minCardDistance / (2.0 * Math.sin((angleStep * Math.PI / 180.0) / 2.0))
  ...
  for (let i = 0; i < partners.length; i++) {
    let angle = (i * 360.0) / partners.length
      ...
      >
        ...
        >
          {partners.map((_, index) => {
            ...
            const x = Math.round((centerX + lineRadius * Math.cos(radians)) * 100) / 100.0
            const y = Math.round((centerY + lineRadius * Math.sin(radians)) * 100) / 100.0
        ...
        {partners.map((partner, index) => {
          ...
          let x = Math.round((centerX + finalRadius * Math.cos(radians)) * 100) / 100.0
          let y = Math.round((centerY + finalRadius * Math.sin(radians)) * 100) / 100.0
...
>
  ...
  >
    {/* iPad 竖版 */}
    <div
      className="ipad-portrait-partners"
      style={{
        width: '100%',
        height: '100%',
        display: 'flex',
    ...
    {/* 手机竖版 */}
    <div
      className="mobile-portrait-partners"
      style={{
        width: '100%',
        height: '100%',
        display: 'flex',
    ...
    {!showMobilePortrait && !showIpadPortrait && (
      <div
        className="ipad-landscape-partners"
        style={{
          width: '100%',
          height: '100%',
          display: 'flex',
